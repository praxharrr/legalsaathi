        ...
        {cases.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {cases.map((c) => (
              <div
                key={c.id}
                className="group relative bg-raised border border-rule rounded-[3px] hover:border-ink hover:shadow-[5px_6px_0_rgba(21,24,28,0.09)] hover:-translate-y-1 transition-all"
              >
                {/* the tape binding the file */}
                <span className="absolute left-0 top-0 bottom-0 w-[3px] bg-tape/70 group-hover:bg-tape transition-colors z-10" />

                <Link href={`/ask/${c.id}`} className="block pl-7 pr-5 py-5">
                  ...
                  <p className="font-serif text-[19px] leading-snug mb-3 line-clamp-2 pr-16">
                    {c.title}
                  </p>
